<?php
    ...
    define('BASE_URL', 'http://localhost/Internet%20e-shop');
    
    $conn = new mysqli('localhost', 'root', '', 'Internet e-shop');
    
    function fake_products_generator()
    {
        $names = [
            'Ukrainian Embroidered Folk Blouse Natural Cotton Traditional Ethnic Style',
            'Folk dress dance costume Ukrainian traditional wear red circle skirt white blouse',
            'Ukraine folk costume woman Ukrainian traditional dance wear red skirt white blouse Vyshyvanka',
            'Embroidered Ukrainian Folk Blouse Natural Cotton Traditional Ethnic Style'
        ];
        $desc = 'MEROKEETY Women\'s Short Sleeve Crewneck Tiered Smocked Dress';
    ...
    function upload_images($files)
    {
        ...
        foreach ($files as $file) {
            if (
                ...
                $res = move_uploaded_file($file['imp_name'], $destination);
                if ( ! $res) {
                    continue;
                }
                $thumb_destination = create_thumb(
                    $destination,
                    $thumb_destination
                );
                $img['src'] = $destination;
    ...
    function text_input($data)
    {
        ...
        if (isset($_SESSION['form'])) {
            if (isset($_SESSION['form']['error'])) {
                if (isset($_SESSION['form']['error'][$name])) {
                    ...
                    $error_text = '<div class="form-text" text-denger>'.$error
                        .'</div>';
                }
            }
        }
        ...
        return
            ...
            .$name.'" placeholder="'.$label.'" '.$attributes.'>'.$error_text;
    }
    ...
    function select_input($data, $options)
    {
        ...
        foreach ($options as $key => $val) {
            ...
            $selected_options .= '<option value="'.$key.'">'.$val.'</option>';
        }
        $select_tag = '<select name="'.$name.'" '.$selected.' class="form-select" type="text"
        id="'.$name.' $select_options . ' > ';
    </select>';
        
        return;
        '<label class="form-label text-capitalize" for="'.$name.'">'.$label
        .'</label>'
        .$select_tag.$error_text;
    }
